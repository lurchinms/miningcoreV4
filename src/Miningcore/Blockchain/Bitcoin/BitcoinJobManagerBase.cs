...
public abstract class BitcoinJobManagerBase<TJob> : JobManagerBase<TJob>
{
    ...
    protected virtual async Task UpdateNetworkStatsAsync(CancellationToken ct)
    {
        try
        {
            var getMiningInfoReq = new RpcRequest(BitcoinCommands.GetMiningInfo);
            var getNetworkInfoReq = new RpcRequest(BitcoinCommands.GetNetworkInfo);
            var getNetworkHashPsReq = new RpcRequest(BitcoinCommands.GetNetworkHashPS);

            var dgbExtraParams = (poolConfig.Template as BitcoinTemplate)?.BlockTemplateRpcExtraParams;
            var isDgbWithParams = dgbExtraParams != null && dgbExtraParams.HasValues && poolConfig.Template.Symbol?.ToUpper() == "DGB";

            if(isDgbWithParams)
            {
                getNetworkHashPsReq = new RpcRequest(BitcoinCommands.GetNetworkHashPS, new object[] { 120, -1, dgbExtraParams.First() });
            }
            ...
            // OR if we requested a specific algo (DGB), use that result
            if(results[2].Error == null && (BlockchainStats.NetworkHashrate == 0 || isDgbWithParams))
            {
    ...
    protected record SubmitResult(bool Accepted, string CoinbaseTx, string RejectReason = null);
    ...
    protected async Task<SubmitResult> SubmitBlockAsync(Share share, string blockHex, CancellationToken ct)
    {
        ...
        logger.Info(() => $"Submitting block {share.BlockHeight} via {(hasSubmitBlockMethod ? "submitblock" : "getblocktemplate")} (hex size: {blockHex.Length / 2} bytes)");

        var submitResponse = await rpc.ExecuteAsync<object>(logger, submitBlockRequest.Method, ct, submitBlockRequest.Payload);
        ...
        var submitError = submitResponse.Error?.Message ??
            ...
            submitResponse.Response?.ToString();

        if(!string.IsNullOrEmpty(submitError))
        {
            logger.Warn(() => $"Block {share.BlockHeight} submission failed with: {submitError}");
            logger.Warn(() => $"Block {share.BlockHeight} submit RPC response - Error: code={submitResponse.Error?.Code}, message={submitResponse.Error?.Message}, Response: {submitResponse.Response}");
            ...
            return new SubmitResult(false, null, $"submission error: {submitError}");
        }

        // Verify acceptance by polling getblock — the daemon may not have the
        // block indexed immediately after submitblock, so a single immediate
        // getblock previously produced false "block not found" failures.
        DaemonResponses.Block block = null;
        RpcResponse<DaemonResponses.Block> acceptResult = null;
        var maxAttempts = hasSubmitBlockMethod ? 5 : 1;

        for(var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            acceptResult = await rpc.ExecuteAsync<DaemonResponses.Block>(logger, BitcoinCommands.GetBlock, ct, new[] { share.BlockHash });
            block = acceptResult.Response;

            if(acceptResult.Error == null && block?.Hash == share.BlockHash)
                break;
        ...
        if(!accepted)
        {
            ...
            logger.Warn(() => $"Block {share.BlockHeight} getblock response - Error: {acceptResult.Error?.Message}, Expected hash: {share.BlockHash}, Got hash: {block?.Hash ?? "(null)"}, Confirmations: {block?.Confirmations.ToString() ?? "(null)"}");
        ...
        {
            logger.Info(() => $"Block {share.BlockHeight} confirmed on chain - hash: {block.Hash}, confirmations: {block.Confirmations}, tx count: {block.Transactions?.Length ?? 0}, coinbaseTx: {block.Transactions?.FirstOrDefault() ?? "(null)"}");
        }

        return new SubmitResult(accepted, block?.Transactions.FirstOrDefault(), accepted ? null : $"block not found after submission (getblock error: {acceptResult.Error?.Message}, returned hash: {block?.Hash ?? "(null)"})");
    }
    ...
    protected override async Task PostStartInitAsync(CancellationToken ct)
    {
        ...
        // Payment-processing setup (address ownership check removed - not needed with zigzag)
